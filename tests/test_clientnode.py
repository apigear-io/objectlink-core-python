...
def test_add_sink():
    ClientNode.add_object_sink(sink)
    assert(r.get_object_sink(name) == sink) 
    assert(r.get_client_node(name) == None)

def test_remove_sink():
    ClientNode.remove_object_sink(sink)
    assert(r.get_object_sink(name) == None) 

def test_link_node_to_sink():
    assert(r.get_client_node(name) == None)
    client.link_remote(name)
    assert(r.get_client_node(name) == client)    
